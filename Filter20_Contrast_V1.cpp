/*
                     ...
                     ** Faculty of Computer and Artificial Intelligence, Cairo University **
                        ----------------------------------------------------------------- 
                                    ...
                                    CS213_Opject Orinted Programing_A1_Part2   
                                                Year 2025-2026
                                                First Semester
                                    
        *1.Name: Abdullah Youssef Abdullah- ID: 20240331 - Implemented filtes 1(Grayscale Conversion) , 4(Merge Images), 
        7(Darken and Lighten Image), 10(Detect Image Edges), 16(Purple effect), 17(Infrared), 18(Skewing image), 19(Saturation),20(Contrast) & The explanatory video.

        *2.Name: Ali Ahmed Mahmoud - ID: 20240337 - Implemented filters 2(Black and White), 5(Flip Image), 8(Crop Image), 11(Resize Image),
        15(TV Screen filter) , The diagram & GitHub repository.

        *3.Name: Yassin Mohamed Abdelaziz - ID: 20240663 - Implemented filters 3(Invert Image), 6(Rotate Image), 9(Add frame),
        12(Blur image), 13(Sun ray), 14(Oil painting picture), The mneu & The main Function.
*/
...
public:
   ...
   Image Skewing(const Image &scr)     // Filter 18    Abdullah Youssef Abdullah 20240331
{
    double angle = 40 * 3.14159265358979323846  / 180;
...
Image Change_Saturation_manually(const Image &scr , double factor)     // Filter 19.1   Abdullah Youssef Abdullah 20240331
{
    Image dst = scr ;
    for (int i = 0; i < dst.width; i++)
    {
        for (int j = 0; j < dst.height; j++)
        {
            ...
            dst(i, j, 0) = clamp(int(gray+(r-gray)*factor),0,255);
            dst(i, j, 1) = clamp(int(gray+(g-gray)*factor),0,255);
            dst(i, j, 2) = clamp(int(gray+(b-gray)*factor),0,255);
...
Image Change_Saturation_automatically(const Image &scr )     // Filter 19.2   Abdullah Youssef Abdullah 20240331
{
    Image dst = scr;
    double total_gray=0;
    ...
    double avgrage_gray = total_gray / total_pixels;
    double factor;
    if (avgrage_gray<60)
    {
    ...
    else if (avgrage_gray<100)
    {
        factor = 1.2;
    }
    else if (avgrage_gray<140)
    {
        factor = 1.00;
    }
    else if (avgrage_gray<180)
    {
        factor = 0.9;
    }
    else if (avgrage_gray<220)
    {
        factor = 0.8;
    }
    else if (avgrage_gray<255)
    {
        factor = 0.7;
    }
     ...
     for (int i = 0; i < dst.width; i++)
    {
        for (int j = 0; j < dst.height; j++)
        {
            ...
            double b = scr(i, j, 2);
            double gray = 0.299 * r + 0.587 * g + 0.114 * b;
            double strength = factor * (1.0 - fabs(gray - 128.0) / 128.0);

            dst(i, j, 0) = clamp(int(gray+(r-gray)*strength),0,255);
            dst(i, j, 1) = clamp(int(gray+(g-gray)*strength),0,255);
            dst(i, j, 2) = clamp(int(gray+(b-gray)*strength),0,255);
...
Image Change_contrast_manually(const Image &scr , double c)     // Filter 20.1   Abdullah Youssef Abdullah 20240331
{
    Image dst=scr;
    double f;
    f=((c+255)*259)/((259-c)*255);
    for (int i = 0; i < dst.width; i++)
    {
        for (int j = 0; j < dst.height; j++)
        {
           int r=scr(i,j,0), g=scr(i,j,1), b=scr(i,j,2);
           dst(i,j,0)=clamp(int(f*(r-128)+128),0,255);
           dst(i,j,1)=clamp(int(f*(g-128)+128),0,255);
           dst(i,j,2)=clamp(int(f*(b-128)+128),0,255);
        }
        
    }
    return dst;
}
Image Change_contrast_automatically(const Image &scr)     // Filter 20.2   Abdullah Youssef Abdullah 20240331
{
    Image dst=scr;
    double total_gray=0,variance=0;
    int total_pixels=dst.width*dst.height;
    for (int i = 0; i < dst.width; i++)
    {
        for (int j = 0; j < dst.height; j++)
    ...
    double average_gray = total_gray / total_pixels;
    for (int i = 0; i < dst.width; i++)
    {
        for (int j = 0; j < dst.height; j++)
        {
            ...
            double b = scr(i, j, 2);
            double gray = 0.299 * r + 0.587 * g + 0.114 * b;
            variance += pow(gray-average_gray,2);
        }
        
    }
    double standard_deviation= sqrt(variance/total_pixels);
    double c;
    
    if (average_gray<80)
    {
        if (standard_deviation<40)
        {
            c=70;
        }
        else
...
void process(int filter_number , Image image , string pictureName) {
    ...
    else if (filter_number == 19)
    {
        cout << "Enter (0) for Changing saturation manually\n";
        cout << "Or enter (1) for changing Saturation automatically\n";
        bool automatic;
        cin >> automatic;
        if (!automatic)
        {
        double factor;
        cout <<"Enter the saturation number (less than 1 to decrease saturation, more than 1 to increase saturation):\n";
        cin >>factor;
        resultImg = editor.Change_Saturation_manually(image,factor);

        }
        else
        {
            resultImg = editor.Change_Saturation_automatically(image);
        }
        

    }
    else if (filter_number == 20)
    {
        cout << "Enter (0) for Changing saturation manually\n";
        cout << "Or enter (1) for changing Saturation automatically\n";
        bool automatic;
        ...
        if (!automatic)
        {
        cout <<"Enter contrast level (-200 to +200): ";
        double c;
        cin>> c ;
        resultImg = editor.Change_contrast_manually(image,c);
        }
        else
        {
            resultImg = editor.Change_contrast_automatically(image);
        }
