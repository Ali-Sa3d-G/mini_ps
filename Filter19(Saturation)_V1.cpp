...
Image increasing_Saturation (const Image &scr , double factor)     // Filter 19.1    Abdullah Youssef Abdullah 20240331
{
    Image dst = scr ;
    for (int i = 0; i < dst.width; i++)
    {
        for (int j = 0; j < dst.height; j++)
        {
            ...
            dst(i, j, 0) = min(int(gray+(r-gray)*factor),255);
            dst(i, j, 1) = min(int(gray+(g-gray)*factor),255);
            dst(i, j, 2) = min(int(gray+(b-gray)*factor),255);
        }
        
    }
    return dst;
}
Image decreasing_Saturation (const Image &scr , double factor)     // Filter 19.2    Abdullah Youssef Abdullah 20240331
{
    Image dst = scr ;
    for (int i = 0; i < dst.width; i++)
    {
        for (int j = 0; j < dst.height; j++)
        {
            ...
            double b = scr(i, j, 2);
            double gray = 0.299 * r + 0.587 * g + 0.114 * b;
            dst(i, j, 0) = max(int(gray+(r-gray)*factor),0);
            dst(i, j, 1) = max(int(gray+(g-gray)*factor),0);
            dst(i, j, 2) = max(int(gray+(b-gray)*factor),0);
        }
...
void process(int filter_number , Image image , string pictureName) {
    ...
    else if (filter_number == 19)
    {
        ...
        cout << "1. increasing Saturation\n";
        cout << "2. decreasing Saturation\n";
        cin >>Increase_or_decrease;
        if (Increase_or_decrease == 1)
        {
            double factor = 1.1;

            resultImg = editor.increasing_Saturation(image,factor);
        }
        else if (Increase_or_decrease == 2)
        {
            double factor = 0.7;
            resultImg = editor.decreasing_Saturation(image,factor);
        }
    }
